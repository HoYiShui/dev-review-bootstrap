...
2. Pick the active task from `state.json.current_task_id` and `plan.yaml.current_task_id`.
3. Spawn `autodev_dev` for that task. Pass the current task goal, the relevant files, and any unresolved findings from `state.json.pending_findings`.
4. Parse the returned JSON. Append a `dev` event to `log.jsonl`, increment `last_event_id`, and set `phase` to `ready_for_review`.
5. If the dev result reports blockers, set `phase` to `blocked`, set `loop_active` to `false`, record the blocker, and stop.
6. Spawn `autodev_reviewer` against the current task, the current diff, and the latest dev result.
7. Parse the returned JSON. Append a `review` event.
8. If verdict is `changes_requested`, keep the same task active, store the findings in `state.json.pending_findings`, set `phase` to `ready_for_dev`, and run another dev-review cycle.
9. If verdict is `accepted`, mark the task `done`, clear `pending_findings`, advance the next `planned` task to `in_progress`, update `current_task_id`, append a `task_advanced` event, and continue.
10. If verdict is `blocked`, set `phase` to `blocked`, set `loop_active` to `false`, record the blocker, and stop.
11. If no planned tasks remain, set `phase` to `done`, set `loop_active` to `false`, append `loop_finished`, and stop.
